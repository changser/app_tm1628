...
////////////////      test_pt1   V180628                                               ////////////
////////////////      by changser    20180702                                         ////////////
////////////////      changser@139.com                                                ////////////

//说明：
//APP_TM1628在协程changser_pt_for_arduino（V20170611）平台上的应用例子2。
...
//          任务3时对应指示灯亮，判断键2是否按下，按下键2后指示灯灭并转到任务2；
//          从指示灯的角度来看，就是用两个指示灯代表两个状态，用键1和键2在两个状态之间切换。

//更改记录：
//V180702       建立，IDE基于1.8.1，APP_TM1628类库版本V0.6，changser_pt_for_arduino版本V20170611
...
static int thread2_entry(struct pt *pt)
{
 ...
 while (1)             //这个循环体可以不要。因为协程切换的本质在于任务被不断调用（比如loop()中），PT_BEGIN(pt);是switch，出让点是case分支。
                         ...
                         //另外，要了的话，看起来像是一个任务，更好理解，以后移植到OS也更方便。
  {
    ...
    {
      if (app_tm1628.keys[0].key_buffer == KEY_DOWN)
      {
...
static int thread3_entry(struct pt *pt)
{
  ...
  while (1)    
  {
    ...
    while (1)
    {
      if (app_tm1628.keys[1].key_buffer == KEY_DOWN)  //如果任务2、3都用一个键的按下动作来判断的话，会出现问题。
